...
def home(request):
    ...
    message_ins = SamatarMessageCounter.objects.get(name='admin')
        ...
        if 'tenant_button' in request.POST:
            ...
            if tenant_form.is_valid():
                ...
                if phone != None:
                        ...
                        message = 'You have a new tenant in your house number {}'.format(selected_unit)
                        cost = 0
                        m = len(message)
                        if m <=144:
                            cost =1
                ...
                messages.error(request, "Error adding tenant. Same name already exists. Name must be unique")
    else:
...
def edit_property(request, id):
    ...
    if request.method == 'POST':
        property_form = PropertyForm(data=request.POST, instance=single_property)
        if property_form.is_valid():
            property_form.save()
...
def edit_landlord(request, id):
    ...
    if request.method == 'POST':
        landlord_form = LandlordForm(data=request.POST, instance=single_landlord)
        if landlord_form.is_valid():
            landlord_form.save()
...
def edit_transfer(request, id):
    ...
    if request.method == 'POST':
        transfer_form = TransferredTenantForm(data=request.POST, instance=single_transfer)
        if transfer_form.is_valid():
            transfer_form.save()
            messages.success(request, "Transferred tenant updated successfully")
        else:
...
def edit_checkout(request, id):
    ...
    if request.method == 'POST':
        checkout_form = CheckedOutTenantForm(data=request.POST, instance=single_checkout)
        if checkout_form.is_valid():
            checkout_form.save()
    ...
    if request.method == 'POST':
        if 'tenant_button' in request.POST:
            ...
            if tenant_form.is_valid():
                ...
                messages.error(request, "Error adding tenant. Same name already exists or input is incorrect")
    else:
        tenant_form = TenantForm()
